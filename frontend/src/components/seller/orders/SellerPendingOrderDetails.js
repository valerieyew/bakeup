...
function SellerPendingOrderDetails(props) {
    ...
    const acceptOrderDetail = () => {
        ...
        new loggedInApi().put("/orders/" + props.orderId, {status: "Processing"}).then(
            res => {
                console.log("closing function is here")
        ...
        <div class="=flex fixed w-7/12 h-3/4 ml-80 mt-32 tracking-wider bg-black">
            <div class="grid grid-rows-5 grid-cols-4 gap-4 h-full p-6 shadow-lg border-2 border-gray-100 bg-white">
                <div class="row-span-2 col-span-4 px-10 grid grid-rows-4 grid-cols-4 items-center">
                    ...
                    <div class="text-md row-span-1 col-span-1 font-bold text-gray-400">CUSTOMER:</div>
                    <div class="text-xl ml-4 row-span-1 col-span-3 text-gray-700">{buyerName}</div>
                ...
                <div class="row-span-1 col-span-4 pt-4">
                    <div class="flex md:px-16 justify-around">
                        <button onClick={(e) => acceptOrderDetail(e)}
                        class="row-span-1 w-48 py-4 text-gray-500 font-bold text-2xl bg-green-100 rounded-md hover:bg-green-300 hover:text-black">ACCEPT</button>
                        <button class="row-span-1 w-48 py-4 text-gray-500 font-bold text-2xl bg-red-100 rounded-md hover:bg-red-300 hover:text-black">DECLINE</button>
                    </div>
                </div>
            </div>
        </div>
...
export default SellerPendingOrderDetails;